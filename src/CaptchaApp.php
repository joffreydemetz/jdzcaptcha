...
use Captcha\CaptchaToken;
use Captcha\Exception\CaptchaException;
use Captcha\Exception\CaptchaTokenException;
// use Captcha\Exception\CaptchaValidationException;
use Captcha\Exception\CaptchaBadRequestException;
...
class CaptchaApp
{
  ...
  public function init()
  {
    ...
    $this->token = new \Captcha\CaptchaToken($this->config->getBool('token'), $this->request->getSession()->get($this->nS.'.csrf'));
  ...
  protected function cleanSessionData()
  {
  ...
  protected function checkIconSet(int $tries=0)
  {
    if ( $tries > 5 ){
      throw new \Exception('JdzCaptcha icon set not found in '.$this->config->get('iconPath'));
    }
  ...
  protected function response(string $content='', int $status=Response::HTTP_OK, array $headers=[])
  {
  ...
  protected function jsonResponse(CaptchaException|CaptchaResponse|array|string $data, int $status=Response::HTTP_OK, array $headers=[])
  {
  ...
  protected function streamResponse($generatedImage, int $status=Response::HTTP_OK, array $headers=[])
  {
